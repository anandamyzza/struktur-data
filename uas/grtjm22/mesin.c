...
/* Prosedur untuk Mencari Jalur Terpanjang */
void searching(char awal, char akhir, char path[], int index, int len, graph G)
{
    ...
    if (awal == akhir) // Jika awal sudah berada di tujuan.
    {
        // Jika panjang jarak sekarang lebih panjang dari jarak terpanjang sebelumnya.
        if (len > jarakTerpanjang)
        {
            jarakTerpanjang = len; // Maka jarak terpanjang sebelumnya diganti menjadi jarak terpanjang sekarang.
        };
    }
    else // Jika awal masih belum di tujuan.
    {
        ...
        while (bantu_jalur != NULL) // Maka Selama Bantu Jalur bukan NULL.
        {
            ...
            if (status == 0) // Jika status bernilai 0, atau belum ada node yang sudah di explore.
            {
                ...
                searching(bantu_jalur->tujuan->kontainer, akhir, path, index, len, G); // Rekursif.
