...
simpul *findSimpul(char tujuan[], simpul *root)
{
    ...
    if (root != NULL)
    {
        if ((strcmp(root->kontainer.name, tujuan) == 0))
        {
            hasil = root;
        }
        else
        {
            ...
            if (bantu != NULL)
            {
                if (bantu->sibling == NULL)
                {
                    ...
                    if ((strcmp(bantu->kontainer.name, tujuan) == 0))
                    {
                        hasil = bantu;
                    }
                    else
...
void printTreePreOrder(simpul *root, simpul *address)
{
    if (root != NULL)
    {
        if (root == address)
        {
            ...
            printf("%s %d", root->kontainer.name, root->kontainer.score);
        }
        else
        {
        ...
        if (bantu != NULL)
        {
            ...
            {
                /*jika memiliki satu simpul anak*/
                printTreePreOrder(bantu, address);
            }
            else
            {
                /*jika memiliki banyak simpul anak*/
...
int isEqual(simpul *root1, simpul *root2)
{
    ...
    {
        if (strcmp(root1->kontainer.name, root2->kontainer.name) != 0 || root1->kontainer.score != root2->kontainer.score)
        {
            hasil = 0;
        }
        else
